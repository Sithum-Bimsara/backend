...
const mapUserToResponse = (user: any): IUser => ({
  ...
  isMaldivesVerified: user.isMaldivesVerified ?? false,
...
export const loginUser = async (req: Request, res: Response) => {
  try {
    ...
    if (error || !data.user) {
      console.error('[Backend] Supabase token verification failed:', error);
      console.error('[Backend] Token prefix:', token?.substring(0, 20));
      return res
        .status(401)
