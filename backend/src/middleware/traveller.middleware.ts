...
/**
 * Optional middleware that attaches residency status (isLocal) to the request.
 * Does not block if user is not logged in or not a traveller.
 */
export const attachResidency = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const user = await getDbUser(req);
    req.isLocal = user?.country === "Maldives";
    next();
  } catch (err) {
    // Fail silently for residency attachment to avoid blocking public routes
    req.isLocal = false;
    next();
  }
};
