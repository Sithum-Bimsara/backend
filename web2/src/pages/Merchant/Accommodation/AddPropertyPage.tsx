...
import PropertyTypeSelector from "../../../features/Accommodation/components/PropertyTypeSelector";
...
import HotelRatingSelector from "../../../features/Accommodation/components/AccommodationForm/Common/HotelRatingSelector";
...
import { useAccommodationOnboarding } from "../../../features/Accommodation/hooks/useAccommodationOnboarding";
import type { CreateAccommodationCompleteDto } from "../../../features/Accommodation/dtos/accommodation.dto";
import FormErrorBanner from "../../../components/FormErrorBanner/FormErrorBanner";

const PropertyTypeScreen: React.FC<{ draft: CreateAccommodationCompleteDto; onChange: (patch: Partial<CreateAccommodationCompleteDto>) => void }> = ({ draft, onChange }) => (
  <div className="space-y-6">
    <div>
      <h2 className="text-2xl font-semibold text-slate-900">Add a new property</h2>
      <p className="mt-2 text-sm text-slate-500">Choose the property type to begin the accommodation onboarding flow.</p>
    </div>
    <PropertyTypeSelector value={draft.type} onChange={(type) => onChange({ type })} />
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
      Apartments include full bedroom composition, homes support entire-place and private-room inventory, hotel/BnB listings support multiple unit types, and alternative stays cover campsites, boats, and luxury tents.
    </div>
  </div>
);
...
const AddPropertyPage: React.FC = () => {
  ...
  const {
    ...
    handleSubmit,
    handleReset,
  } = useAccommodationOnboarding();

  const renderStep = () => {
    const activeStep = steps[currentStep];
    switch (activeStep) {
      case "type":
        return <PropertyTypeScreen draft={draft} onChange={patchDraft} />;
      case "location":
        return <AddressForm value={draft} onChange={patchDraft} />;

      case "details":
        return (
          <div className="space-y-4">
            <PropertyNameForm name={draft.name} description={draft.description} onChange={(patch) => patchDraft(patch)} />
            <HotelRatingSelector value={draft.starRating} onChange={(starRating) => patchDraft({ starRating })} />
          </div>
        );
      case "house-rules":
        return (
          <HouseRulesForm
            ...
            onChange={(houseRules) => patchDraft({ houseRules })}
          />
        );
      case "languages":
        return (
          <div className="space-y-4">
            <LanguagesSelector value={draft.languages} onChange={(languages) => patchDraft({ languages })} />
          </div>
        );
      case "host-profile":
        return <HostProfileForm value={draft.hostProfile} onChange={(hostProfile) => patchDraft({ hostProfile })} />;
      case "facilities":
        return (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
            <div className="space-y-10">
              <FacilitiesSelector value={draft.propertyFacilities} onChange={(propertyFacilities) => patchDraft({ propertyFacilities })} />

              <div className="pt-10 border-t border-slate-100 space-y-4">
                <h3 className="text-xs font-black text-[#0e2a47] uppercase tracking-widest opacity-50">Operational Services</h3>
                <ServicesForm value={draft.services} onChange={(services) => patchDraft({ services })} />
              </div>
            </div>
          </div>
        );
      case "nearby":
        return (
          <NearbyPOIForm
            value={draft.nearbyPointsOfInterest}
            onChange={(nearbyPointsOfInterest) => patchDraft({ nearbyPointsOfInterest })}
          />
        );
      case "marine-life":
        return (
          <MarineLifeSelector
            value={draft.marineLifeZones}
            onChange={(marineLifeZones) => patchDraft({ marineLifeZones })}
          />
        );
      case "units":
        return <PrivateRoomManager units={draft.units} onChange={(units) => patchDraft({ units })} homeListingType={draft.homeListingType} />;
      case "images":
        return <ImageUploader value={draft.images} onChange={(images) => patchDraft({ images })} />;
      case "final":
        return (
          <FinalStep
            agreementAccepted={draft.agreementAccepted}
            activateListing={draft.activateListing}
            onChange={(patch) => patchDraft({
              ...patch,
              agreementAccepted: patch.agreementAccepted !== undefined ? (patch.agreementAccepted as true) : undefined
            })}
          />
        );
      default:
        ...
        <div className="space-y-6 md:space-y-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm relative">
            <div className="p-5 md:p-10">
              ...
              <div className="min-h-50 md:min-h-75">
                <AnimatePresence mode="wait">
                  <motion.div key={steps[currentStep]} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }}>
                    {renderStep()}
                  </motion.div>
                </AnimatePresence>
              </div>
            ...
            <div className="px-5 md:px-8 py-4 md:py-6 bg-slate-50 border-t border-slate-200 flex items-center justify-between rounded-b-xl">
              ...
              >
                {loading ? (
                  ...
                  <>
                    {currentStep === steps.length - 1 ? 'Publish Property' : 'Continue'}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div className="p-4 md:p-6 rounded-xl border border-slate-200 bg-white/50 space-y-2">
              ...
              <p className="text-[10px] md:text-[11px] text-slate-500 leading-relaxed font-medium">Our support team is available 24/7 to help you optimize your property listing for maximum bookings.</p>
            </div>
          </div>
        </div>
      </main>
