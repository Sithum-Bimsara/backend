...
import type { IDealLock } from '../types/deals.types';
...
export const DealLockCard: React.FC<Props> = ({ lock }) => {
  ...
  const handleChat = async () => {
    ...
    try {
      await initiateChat({ dealLockId: lock.id });
    } finally {
    ...
    <div className={`bg-white rounded-2xl border shadow-sm transition-all hover:shadow-md p-5 ${isExpired && !isConverted ? 'border-red-100 opacity-80' : 'border-slate-100'}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <div className="bg-slate-50 rounded-xl p-3">
          ...
          <p className="text-xs font-bold text-[#0e2a47] truncate">{lock.dealTitle}</p>
        </div>
        <div className="bg-slate-50 rounded-xl p-3">
          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1">Slots Locked</p>
          <p className="text-xs font-bold text-[#0e2a47]">{lock.quantity} Slot{lock.quantity !== 1 ? 's' : ''}</p>
        </div>
        <div className="bg-slate-50 rounded-xl p-3">
