...
import type { IAccommodationBooking } from '../types/accommodation.types';
import { useChat } from '../../Chat/ChatContext';

interface Props {
  booking: IAccommodationBooking;
}
...
const AccommodationBookingCard: React.FC<Props> = ({ booking }) => {
  ...
  const handleChat = async () => {
    ...
    try {
      await initiateChat({ accommodationLockId: booking.lockId });
    } finally {
    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm transition-all hover:shadow-md p-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
        <div className="bg-slate-50 rounded-xl p-3">
          ...
          <p className="text-xs font-bold text-[#0e2a47] truncate">{booking.unitName}</p>
        </div>
        <div className="bg-slate-50 rounded-xl p-3">
        ...
        <div className="bg-slate-50 rounded-xl p-3">
          ...
          <p className="text-xs font-bold text-[#0e2a47]">{formatDate(booking.checkOutDate)}</p>
        </div>
      </div>
